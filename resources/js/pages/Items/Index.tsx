...
import { create, show, edit, destroy } from '@/actions/App/Http/Controllers/ItemController';
...
export default function Index({ items }: { items: Item[] }) {
    ...
    return (
        <AppLayout breadcrumbs={breadcrumbs}>
            <Head title="Items" />
            <div className="flex">
                <div className="flex items-center text-2xl">Items</div>
                <div className="ml-auto flex items-center">
                    <CustomLink label="Add item" href={create()} />
                </div>
            </div>
                            ...
                            <Table className="min-w-2xl">
                                <TableHeader>
                                    <TableRow className="hover:bg-transparent!">
                                        ...
                                        <TableHead className="p-2">Available Stock</TableHead>
                                ...
                                <TableBody className="dark:divide-darkborder divide-y divide-border">
                                    {items.map((item, index) => (
                                        <TableRow key={index}>
                                            ...
                                            <TableCell className="whitespace-nowrap">
                                                <h6 className="text-sm font-medium">{item.category?.name}</h6>
                                            </TableCell>
                                            <TableCell className="whitespace-nowrap">
                                                <h6 className="text-sm font-medium">{item.available_stock}</h6>
                                            </TableCell>
                                            <TableCell className="whitespace-nowrap">
