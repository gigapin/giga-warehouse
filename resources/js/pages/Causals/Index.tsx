...
import { create, show, edit, destroy } from '@/actions/App/Http/Controllers/CausalController';
...
export default function Index({ causals }: { causals: Causal[] }) {
    const dateString = (item: Date) => new Date(item).toISOString().split('T')[0];
    return (
        <AppLayout breadcrumbs={breadcrumbs}>
            <Head title="Causals" />
            <div className="flex">
                <div className="flex items-center text-2xl">Causals</div>
                <div className="ml-auto flex items-center">
                    <CustomLink label="Add causal" href={create()} />
                </div>
            </div>
                            ...
                            <Table className="min-w-2xl">
                                <TableHeader>
                                    <TableRow className="hover:bg-transparent!">
                                        ...
                                        <TableHead className="p-2">Typology</TableHead>
                                    ...
                                    {causals.map((item, index) => (
                                        <TableRow key={index}>
                                            ...
                                            <TableCell className="whitespace-nowrap">
                                                <h6 className="text-sm font-medium">{item.code}</h6>
                                            </TableCell>
                                            <TableCell className="whitespace-nowrap">
                                                <h6 className="text-sm font-medium">{item.typology}</h6>
                                            </TableCell>
                                            <TableCell className="whitespace-nowrap">
