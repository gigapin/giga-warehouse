...
import { create, show, edit, destroy } from '@/actions/App/Http/Controllers/MunicipalityController';
...
export default function Index({ municipalities }: { municipalities: Municipality[] }) {
    const dateString = (item: Date) => new Date(item).toISOString().split('T')[0];
    return (
        <AppLayout breadcrumbs={breadcrumbs}>
            <Head title="Municipalities" />
            <div className="flex">
                <div className="flex items-center text-2xl">Municipalities</div>
                <div className="ml-auto flex items-center">
                    <CustomLink label="Add municipality" href={create()} />
                </div>
            </div>
                            ...
                            <Table className="min-w-2xl">
                                <TableHeader>
                                    <TableRow className="hover:bg-transparent!">
                                        ...
                                        <TableHead className="p-2">Postal Code</TableHead>
                                        <TableHead className="p-2">Region</TableHead>
                                ...
                                <TableBody className="dark:divide-darkborder divide-y divide-border">
                                    {municipalities.map((item, index) => (
                                        <TableRow key={index}>
                                            ...
                                            <TableCell className="whitespace-nowrap">
                                                <h6 className="text-sm font-medium">{item.district}</h6>
                                            </TableCell>
                                            <TableCell className="whitespace-nowrap">
                                                <h6 className="text-sm font-medium">{item.postal_code}</h6>
                                            </TableCell>
                                            <TableCell className="whitespace-nowrap">
                                                <h6 className="text-sm font-medium">{item.region}</h6>
                                            </TableCell>
                                            <TableCell className="whitespace-nowrap">
