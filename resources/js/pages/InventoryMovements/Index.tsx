...
import { create, show, edit, destroy } from '@/actions/App/Http/Controllers/InventoryMovementController';
...
export default function Index({ inventoryMovements }: { inventoryMovements: InventoryMovement[] }) {
    const dateString = (item: Date) => new Date(item).toISOString().split('T')[0];
    return (
        <AppLayout breadcrumbs={breadcrumbs}>
            <Head title="Inventory Movements" />
            <div className="flex">
                <div className="flex items-center text-2xl">Inventory Movements</div>
                <div className="ml-auto flex items-center">
                    <CustomLink label="Add inventory movement" href={create()} />
                </div>
            </div>
                            ...
                            <Table className="min-w-2xl">
                                <TableHeader>
                                    <TableRow className="hover:bg-transparent!">
                                        ...
                                        <TableHead className="p-2">Causal</TableHead>
                                ...
                                <TableBody className="dark:divide-darkborder divide-y divide-border">
                                    {inventoryMovements.map((item, index) => (
                                        <TableRow key={index}>
                                            ...
                                            <TableCell className="whitespace-nowrap">
                                                <h6 className="text-sm font-medium">{item.causal?.code}</h6>
                                            </TableCell>
                                            <TableCell className="whitespace-nowrap">
                                                <h6 className="text-sm font-medium">{item.inventory_movement_type}</h6>
                                            </TableCell>
                                            <TableCell className="whitespace-nowrap">
                                                <h6 className="text-sm font-medium">{item.quantity}</h6>
                                            </TableCell>
                                            <TableCell className="whitespace-nowrap">
                                                <h6 className="text-sm font-medium">{item.inventory_movement_date}</h6>
                                            </TableCell>
                                            <TableCell className="whitespace-nowrap">
